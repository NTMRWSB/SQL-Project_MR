...
class AddFrame ( wx.Frame ):
    ...
    def __init__( self, parent ):
        ...
        for i in range(len(col_names)):
            ...
            self.m_textCtrls[i].Value=data[i]
            if i==0:
                self.m_textCtrls[i].SetBackgroundColour( wx.SystemSettings.GetColour( wx.SYS_COLOUR_INACTIVECAPTION ) )
            bSizers[i].Add( self.m_textCtrls[i], 1, wx.ALL, 5 )
            bSizer.Add( bSizers[i], 1, wx.EXPAND, 5 )

        self.m_button = wx.Button( self, wx.ID_ANY, u"修改", wx.DefaultPosition, wx.DefaultSize, 0 )
